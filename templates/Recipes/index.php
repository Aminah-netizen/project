		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars-staggered"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Recipe'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="col-md-9">
		...
		<div class="nav-align-top mb-4">
			<ul class="nav nav-tabs nav-fill border-bottom mb-4" role="tablist">
				...
				<li class="nav-item">
    			<a class="nav-link disabled" aria-disabled="true"><i class="fa-solid fa-file-export"></i> Export</a>
 				 </li>
			</ul>
		</div>
		...
		?>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= $recipe->hasValue('user') ? $this->Html->link($recipe->user->id, ['controller' => 'Users', 'action' => 'view', $recipe->user->id]) : '' ?></td>
                    <td><?= h($recipe->name) ?></td>
                    <td><?= $recipe->hasValue('category') ? $this->Html->link($recipe->category->name, ['controller' => 'Categories', 'action' => 'view', $recipe->category->id]) : '' ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							...
							<?php if ($this->Identity->isLoggedIn() && $this->Identity->get('user_group_id')) : ?>
								<?= $this->Html->link(
									__('<i class="fa-regular fa-pen-to-square"></i>'),
									['action' => 'edit', $recipe->id],
									['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]
								) ?>
								...
								<?= $this->Form->postLink(
									...
									[
										'confirm' => __('Are you sure you want to delete Recipes: "{0}"?', $recipe->id),
										'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_recipes_active; ?></h3>
					<p>Active Recipes</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_recipes_archived; ?></h3>
					<p>Archived Recipes</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Recipes (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Recipes by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Recipes(s)',
			data: [<?= json_encode($total_recipes_active); ?>, <?= json_encode($total_recipes_disabled); ?>, <?= json_encode($total_recipes_archived); ?>],
            backgroundColor: [
    ...
    <div class="col-md-3">
        <div class="card bg-body-tertiary border-0 shadow mb-4">
            <div class="card-body">
                ...
                <?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Recipes', 'action' => 'index']]); ?>
                    <fieldset>
                        <div class="mb-1">
                    ...
                    <div class="text-end">
                        <?php 
                            if (!empty($_isSearch)) {
                                echo $this->Html->link(
                                    __('Reset'),
                                    ['action' => 'index', '?' => array_intersect_key($this->request->getQuery(), array_flip(['sort', 'direction']))],
        ...
        <div class="card bg-body-tertiary border-0 shadow mb-4">
            <div class="card-body">
                <div class="text-center">
					...
					<i class="fa-solid fa-download"></i> Download Recipe Book
                    </button>
                </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Select Recipe Book Theme</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">

							
							<?= $this->Html->link('Default Theme', ['action' => 'pdfAll', '?' => ['theme' => 'default']], ['class' => 'btn btn-sm btn-outline-secondary']); ?>
							<?= $this->Html->link('Classic Theme', ['action' => 'pdfAll', '?' => ['theme' => 'classic']], ['class' => 'btn btn-sm btn-outline-info']); ?>
							<?= $this->Html->link('Modern Theme', ['action' => 'pdfAll', '?' => ['theme' => 'modern']], ['class' => 'btn btn-sm btn-outline-light']); ?>
	...
	<div class="col-md-12">
		...
		<div class="dsline mb-3"></div>
		<?= $this->Html->image('githubqr.png', ['alt' => '', 'class' => '', 'style' => 'width: 150px; height: 150px;']); ?>
        </div>
		</div>
    </div>
</div>
